  ...
  return (
    <div className="container mx-auto px-4 py-16 max-w-7xl animate-in fade-in duration-700">
      <div className="flex flex-col items-center text-center space-y-4 mb-16">
         <Badge variant="outline" className="px-4 py-1 border-primary/20 bg-primary/5 text-primary text-xs uppercase tracking-widest font-bold">
         ...
         <p className="text-muted-foreground max-w-2xl text-lg leading-relaxed">
            Accelerate your career with expertly curated learning tracks designed by highly qualified industry professionals and mentors.
          </p>
      </div>

      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {templates.map((t: PathTemplate & { mentor: User }) => (
           <Card key={t.id} className="flex flex-col h-full overflow-hidden transition-all hover:shadow-lg border-border/50 group bg-card/50 backdrop-blur-sm">
              <CardHeader className="pb-4">
                 <div className="flex items-center justify-between mb-3 text-xs">
                    <Badge variant="secondary" className="uppercase font-semibold tracking-wider font-mono bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
                      {t.level}
                    </Badge>
                 ...
                 <Link href={`/templates/${t.slug}`}>
                    <CardTitle className="text-xl group-hover:text-primary transition-colors line-clamp-1">{t.title}</CardTitle>
                 </Link>
                 <div className="flex items-center gap-2 mt-2 pt-1">
                    ...
                    <span className="text-xs text-muted-foreground font-medium">By {t.mentor.name}</span>
              ...
              <CardFooter className="pt-0 pb-6 pr-6 justify-end">
                 <Button asChild variant="ghost" className="group/btn font-semibold hover:bg-primary/5 hover:text-primary">
                    <Link href={`/templates/${t.slug}`}>
                      Initialize Path <ArrowRight className="w-4 h-4 ml-2 group-hover/btn:translate-x-1 transition-transform" />
                    </Link>
                 </Button>
              </CardFooter>
           </Card>
