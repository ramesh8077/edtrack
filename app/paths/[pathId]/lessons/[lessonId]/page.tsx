...
export default async function LessonPage({ params }: { params: Promise<{ pathId: string, lessonId: string }> }) {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full p-6 lg:p-10 pb-24">
      <div className="mb-8 space-y-2">
        ...
        <h1 className="text-3xl lg:text-4xl font-heading font-bold tracking-tight">{lesson.title}</h1>
        <div className="flex items-center gap-4 text-sm text-muted-foreground pt-2">
           <span>⏱ {lesson.estimatedMinutes} min read</span>
           {lesson.completedAt && <span className="text-primary font-medium flex items-center gap-1">✓ Completed</span>}
        </div>
      </div>
