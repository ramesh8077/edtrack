...
export default async function DashboardPage({ searchParams }: DashboardProps) {
  ...
  const statusFilter = awaitedSearchParams.status
    ? (awaitedSearchParams.status.toUpperCase() as PathStatus)
    : undefined;
  const levelFilter = awaitedSearchParams.level
    ? (awaitedSearchParams.level.toUpperCase() as Level)
    : undefined;
        ...
        <div className="md:col-span-3 space-y-6">
          <div className="flex items-center justify-between border-b pb-2">
            ...
            <div className="flex gap-4 text-sm text-muted-foreground">
              ...
              <Link
                href="/dashboard"
                className={`hover:text-primary ${!statusFilter ? "font-medium text-foreground" : ""}`}
              >
                All
              </Link>
              <Link
                href="/dashboard?status=active"
                className={`hover:text-primary ${statusFilter === "ACTIVE" ? "font-medium text-foreground" : ""}`}
              >
                Active
              </Link>
              <Link
                href="/dashboard?status=completed"
                className={`hover:text-primary ${statusFilter === "COMPLETED" ? "font-medium text-foreground" : ""}`}
              >
                Completed
              </Link>
            </div>
          ...
          {paths.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-12 text-center rounded-xl border border-dashed bg-muted/10">
              ...
              <p className="text-muted-foreground mt-1 max-w-sm mb-4">
                You haven&apos;t instantiated any learning tracks yet. Generate a path directly
                using AI or enroll from a verified mentor.
              </p>
              <Button asChild>
                <Link href="/paths/new">Create Path</Link>
              </Button>
            </div>
          ) : (
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-xs text-muted-foreground -mt-3 mb-2">
              Minutes studied over last 30 assessments
            </p>
