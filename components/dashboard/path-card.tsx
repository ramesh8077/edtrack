    ...
    <Card className="flex flex-col overflow-hidden transition-all hover:shadow-md border-border/50 bg-card/60 backdrop-blur-sm">
      <CardHeader className="pb-4">
        <div className="flex items-start justify-between mb-2">
          <Badge variant={getStatusColor(path.status) as "default" | "outline" | "secondary"} className="text-[10px] font-semibold uppercase tracking-wider">
            {path.status}
          </Badge>
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${getLevelColor(path.level)}`}>
            {path.level}
          </span>
        </div>
        <CardTitle className="text-xl line-clamp-2 leading-tight group-hover:text-primary transition-colors">
          <Link href={`/paths/${path.id}`} className="hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-sm">
