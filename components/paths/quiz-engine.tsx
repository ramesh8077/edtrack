...
export function QuizEngine({ quiz: rawQuiz, pathId }: { quiz: Record<string, unknown>; pathId: string }) {
  const quiz = rawQuiz as unknown as Quiz;
  const [answers, setAnswers] = useState<Record<string, string | string[]>>({});
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [result, setResult] = useState<{ score: number; passed: boolean; aiFeedback?: string; itemizedFeedback?: Record<string, string> } | null>(null);

  const handleSelect = (questionId: string, val: string, type: string) => {
    if (type === "MSQ") {
      setAnswers((prev) => {
        const current = (prev[questionId] as string[]) || [];
        if (current.includes(val)) return { ...prev, [questionId]: current.filter((x) => x !== val) };
  ...
  const onSubmit = async () => {
    ...
    try {
        ...
        if (q.type === "OPEN") {
          ...
          const res = await fetch("/api/ai/grade", {
            ...
            body: JSON.stringify({
              question: q.prompt,
              answer: ans,
              rubric: q.correctAnswer || "Provide reasonable, historically/technically accurate analysis.",
            }),
          });
          ...
          if (q.type === "MCQ") {
             ...
             if (ansArr.length === correctArr.length && ansArr.every(x => correctArr.includes(x))) {
               totalScore += 10;
             }
          }
      ...
      setResult({ score: finalPercentage, passed, aiFeedback: combinedAiFeedback, itemizedFeedback: itemizedDetails });
    ...
    return (
      <Card className="border-border/50 bg-card p-6 shadow-md">
         <div className="flex flex-col items-center justify-center text-center py-6 border-b border-border/40">
            {result.passed ? <CheckCircle2 className="h-16 w-16 text-emerald-500 mb-4" /> : <XCircle className="h-16 w-16 text-destructive mb-4" />}
            <h2 className="text-3xl font-bold">{result.score}%</h2>
            <p className="text-muted-foreground mt-2">{result.passed ? "Congratulations, you passed the assessment!" : "You did not reach the passing threshold. Don't worry, read the feedback and try again."}</p>
         </div>
         ...
         <div className="py-6 space-y-6">
            ...
            {quiz.questions.map((q: QuizQuestion, i: number) => (
              <div key={q.id} className="p-4 rounded-lg bg-muted/30 border border-border/50 text-sm">
                <p className="font-medium mb-2">Q{i + 1}: {q.prompt}</p>
  ...
  return (
    <div className="space-y-10">
         ...
         return (
           <Card key={q.id} className="border-border/50 bg-card overflow-hidden">
             <div className="bg-muted/30 p-4 border-b border-border/40">
               <span className="text-xs font-bold uppercase text-muted-foreground tracking-widest">Question {i + 1}</span>
             ...
             <CardContent className="p-4 pt-6 space-y-3">
               {(isMCQ || isMSQ) ? (
                 <div className="space-y-2">
                   {options.map((opt) => {
                     const isSelected = isMSQ ? (answers[q.id] as string[] || []).includes(opt) : answers[q.id] === opt;
                     return (
                       <div 
                         key={opt}
      ...
      <div className="flex justify-end pt-4 border-t border-border/40">
        <Button onClick={onSubmit} disabled={isSubmitting} size="lg" className="w-full sm:w-auto font-semibold">
