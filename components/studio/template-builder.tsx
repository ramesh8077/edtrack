...
export function TemplateBuilder({ initialData }: { initialData: any }) {
  ...
  const addModule = () => {
    setModules([
      ...modules,
      { id: crypto.randomUUID(), title: "New Module", description: "", lessons: [] },
    ]);
  };
  ...
  const togglePublish = async () => {
    ...
    if (res.success)
      toast.success(
        `Template ${!initialData.isPublished ? "Published! Live on marketplace." : "Unpublished."}`,
      );
        ...
        <div className="flex gap-4 items-center">
          ...
          <div>
            ...
            <span className="text-xs text-muted-foreground uppercase">
              {initialData.isPublished ? "Published" : "Draft Mode"}
            </span>
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        <div className="space-y-4 md:col-span-1 border-r border-border/40 pr-6">
          ...
          <Textarea
            ...
            placeholder="Curriculum Goal & Syllabus details..."
              ...
              <div className="space-y-4">
                {modules.map((module, mIndex) => (
                  <SortableItem
                    key={module.id}
                    id={module.id}
                    className="rounded-md border border-border/60 bg-card overflow-hidden shadow-sm"
                  >
                      ...
                      <div className="p-3 bg-muted/20 border-b border-border/40 flex items-center justify-between gap-4">
                        <Input
                          ...
                          onChange={(e) =>
                            setModules((prev) =>
                              prev.map((p) =>
                                p.id === module.id ? { ...p, title: e.target.value } : p,
                              ),
                            )
                          }
                        ...
                        <Button
                          size="icon"
                          variant="ghost"
                          className="text-destructive h-8 w-8 shrink-0 hover:bg-destructive/10 hover:text-destructive"
                          onClick={() =>
                            setModules((prev) => prev.filter((p) => p.id !== module.id))
                          }
                        >
                      ...
                      <div className="p-4 space-y-4 shadow-inner bg-background/50">
                            ...
                            <div className="bg-muted/10 p-2 border-b flex justify-between">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  setModules((prev) => {
                                    const newM = [...prev];
                                    newM[mIndex]!.lessons = newM[mIndex]!.lessons.filter(
                                      (_, i: number) => i !== lIndex,
                                    );
                                    return newM;
                                  });
                        ...
                        <Button
                          ...
                          onClick={() => {
                            setModules((prev) => {
                              const newM = [...prev];
                              newM[mIndex]!.lessons.push({
                                id: crypto.randomUUID(),
                                title: "New Resource",
                                contentMd: "<p>Start writing here...</p>",
                              });
                              return newM;
                            });
                          }}
